/**
 * Created by everton.ferreira on 04/09/2017.
 */
function Weapons(_game, _player){
...
Weapons.prototype.create = function __create(){

    //  Creates 30 bullets, using the 'bullet' graphic
    this.weapon = this.game.add.weapon(30, 'weapon');
    ...
    this.weapon.outOfBoundsKill = true;
    ...
    this.weapon.trackSprite(this.player, 32, 16, true);
...
Weapon.SingleBullet = function (_this) {

    Phaser.Group.call(this, _this.game, _this.game.world, 'Single Bullet', false, true, Phaser.Physics.ARCADE);
    ...
    for (var i = 0; i < 64; i++)
    {
        this.add(new Bullet(_this.game, 'weapon'), true);
    }
...
Weapon.SingleBullet.prototype.fire = function (source, angleName, tileSize, damage) {

    if (this.game.time.time < this.nextFire) { return; }

    var x = source.x + (tileSize/2);
    var y = source.y + (tileSize/2);
    var angle = 0;

    if(angleName == "left" || angleName == "stopLeft") angle = 180;
    else if(angleName == "up" || angleName == "stopUp") angle = 270;
    else if(angleName == "down" || angleName == "stopDown") angle = 90;
